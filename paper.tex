...
\begin{eqnarray*}
  ...
  + \sum_{\missing} P(\missing | \weights^{(\emiteration-1)}, \allrealdata, \allcatlabels) \left[ \log P(\missing| \weights, \allrealdata) + \log P(\allcatlabels|\labelerrprobs,\missing) \right]
  \\
  & = & 
  \log P(\weights) + \log P(\labelerrprobs)
  \\ & &
